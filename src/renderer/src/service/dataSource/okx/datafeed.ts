  ...
  implements TradingView.IExternalDatafeed, TradingView.IDatafeedChartApi
{
  ...
  private async getAllSymbols() {
    ...
    const data = await makeOKXRequest('api/v5/public/instruments?instType=SPOT')

    let allSymbols: any[] = []

    // OKX API 返回格式: { code: "0", msg: "", data: [...] }
    if (data.code !== '0' || !data.data) {
      console.error('[getAllSymbols]: API error', data)
      return []
    }
    ...
    for (const exchange of configurationData.exchanges!) {
      ...
      for (const pair of pairs) {
        // OKX 返回的字段: instId, baseCcy, quoteCcy, tickSz 等
  ...
  public async resolveSymbol(
    ...
    if (!symbolItem) {
      // 如果 symbolName 不包含 ":"，尝试添加 "OKX:" 前缀
      if (!symbolName.includes(':')) {
        const normalizedName = `OKX:${symbolName}`
    ...
    const symbolInfo: Partial<TradingView.LibrarySymbolInfo> = {
      ticker: symbolItem.full_name, // 始终使用完整格式 "OKX:BTC/USDT"
  ...
  public async getBars(
    ...
    const interval = OKX_RESOLUSION[resolution as keyof typeof OKX_RESOLUSION]
    if (!interval) {
    ...
    try {
      console.log(`[getBars]: Requesting ${parsedSymbol.instId} ${interval} from ${new Date(from * 1000).toISOString()} to ${new Date(to * 1000).toISOString()}`)
      console.log(`[getBars]: URL: api/v5/market/candles?${query}`)
