import { ChartData, ChartMetaInfo, IExternalSaveLoadAdapter } from '@public/charting_library'
...
export const save_load_adapter: SaveLoadAdapter = {
  ...
  removeChart: function (id: string | number) {
  ...
  getChartContent: function (id: string | number) {
    for (let i = 0; i < this.charts.length; ++i) {
      if (this.charts[i].id === id) {
  ...
  removeStudyTemplate: function (studyTemplateData: { name: string }) {
  ...
  getStudyTemplateContent: function (studyTemplateData: { name: string }) {
    for (let i = 0; i < this.studyTemplates.length; ++i) {
      if (this.studyTemplates[i].name === studyTemplateData.name) {
  ...
  saveStudyTemplate: function (studyTemplateData: { name: string }) {
    for (let i = 0; i < this.studyTemplates.length; ++i) {
      if (this.studyTemplates[i].name === studyTemplateData.name) {
        this.studyTemplates.splice(i, 1)
        break
  ...
  getDrawingTemplates: function () {
    return Promise.resolve(
      this.drawingTemplates.map(function (template: { name: string }) {
  ...
  async getAllChartTemplates() {
    return this.chartTemplates.map((x: { name: string }) => x.name)
  },

  async saveChartTemplate(templateName: any, content: any) {
    const theme = this.chartTemplates.find((x: { name: string }) => x.name === templateName)
  ...
  async removeChartTemplate(templateName: any) {
    this.chartTemplates = this.chartTemplates.filter(
      (x: { name: string }) => x.name !== templateName
    )
  },
