...
- [x] **3.6** 指标 Tooltip 操作按钮
  - 每个指标名旁显示 3 个图标按钮（眼睛=显隐、齿轮=参数设置、×=移除）
  - 通过 `createTooltipDataSource` + `subscribeAction(OnTooltipIconClick)` 实现
  - 指标参数设置弹窗：支持所有 27 个指标的 calcParams 编辑，确认后 `overrideIndicator` 实时更新
